...
class RecommendationEngineTest < ActiveSupport::TestCase
  setup do
    ...
    12.times do |index|
      Book.create!(isbn: "978400000#{format('%03d', index)}#{index % 10}", title: "候補#{('A'.ord + index).chr}", author: "著者#{index / 2}", categories: [ index.even? ? "SF" : "科学" ], page_count: 280 + index * 10, description: "科学と社会を物語で考える本", recommendable: true)
    end
  ...
  test "creates distinct categories for all algorithms and excludes read books" do
    ...
    Recommendation::ALGORITHMS.each do |algorithm|
      ...
      assert_equal results.includes(:book).map { |result| result.book.series_key }.uniq, results.includes(:book).map { |result| result.book.series_key }
  ...
  test "books rejected by the reader are excluded from later sessions" do
    session = RecommendationEngine.new(user: @user, profile: @profile, goal: "same_pace").call
    recommendation = session.recommendations.first
    recommendation.feedbacks.create!(feedback_type: "not_for_me")
